...
<div class="tab-pane fade " id="refundPayment-tab-pane" role="tabpanel" aria-labelledby="refundPayment-tab" tabindex="0">
    ...
    <h2 class="content-title">Refund Payment</h2>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="text-center" width="20%">
                                {{ trans('rest.settings.payment.reason') }} </th>
                    ...
                    <tbody>
                        @foreach ($refundRequests as $key => $order)
                            <tr>
                                ...
                                <td class="text-center">
                                    <div
                                        class="d-flex align-items-center gap-3 flex-lg-wrap flex-xl-nowrap justify-content-center refund_status_box_{{ $order->id }}">
                                        @if ($order->refund_status == RefundStatus::Pending)
                                            <a class="btn btn-site-theme btn-default d-block px-2 py-1 rounded-2"
                                                onclick="changeRefundStatus({{ $order->id }},'2')">
                                                <span class="align-middle"
                                                    style="font-size: 11px;">{{ trans('rest.settings.payment.reject') }}</span>
                                            </a>
                                            <a class="btn btn-site-theme btn-default d-block px-2 py-1 rounded-2"
                                                onclick="changeRefundStatus({{ $order->id }},'1')">
                                                <span class="align-middle"
                                                    style="font-size: 11px;">{{ trans('rest.settings.payment.accept') }}</span>
                                            </a>
                                        @else
