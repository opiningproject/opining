    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header border-0">
                <div class="head-flex">
                    <h3 class="mb-0">{{ $order->order_type == OrderType::Delivery ? trans('rest.food_order.delivery') : trans('rest.food_order.pickup') }} ORDER #{{ $order->id }}</h3>
            ...
            <div class="modal-body pt-1 pb-0">
                <div class="border-0 d-flex align-items-center justify-content-between mb-0">
                    ...
                    <div class="order-status">
                        ...
                        <div class="order-col">
                            <label class="status-option order-status-option {{ $order->order_status == OrderStatus::Accepted ? "active" : "" }}">
                                <input id="test1" type="radio" name="orderStatus" {{ $order->order_status == OrderStatus::Accepted ? "checked" : "" }} />
                                <span for="test1"></span>
                        ...
                        <div class="order-col">
                            <label class="order-status-option status-option {{ $order->order_status == OrderStatus::InKitchen ? "active" : "" }}">
                                <input id="test2" type="radio" name="order-status-option" {{ $order->order_status == OrderStatus::InKitchen ? "checked" : "" }} onclick="changeOrderStatusNew({{ $order->id }},'{{ OrderStatus::InKitchen }}')" />
                                <span for="test2"></span>
                        ...
                        <div class="order-col">
                            <label class="order-status-option status-option {{ $order->order_status == OrderStatus::OutForDelivery ? "active" : "" }}">
                                <input id="test3" type="radio" name="order-status-option" {{ $order->order_status == OrderStatus::OutForDelivery ? "checked" : "" }} onclick="changeOrderStatusNew({{ $order->id }},'{{ OrderStatus::OutForDelivery }}')" />
                                <span for="test3"></span>
                        ...
                        @else
                        <div class="order-col">
                            <label class="order-status-option status-option {{ $order->order_status == OrderStatus::ReadyForPickup ? "active" : "" }}">
                                <input id="test3" type="radio" name="order-status-option" {{ $order->order_status == OrderStatus::ReadyForPickup ? "checked" : "" }} onclick="changeOrderStatusNew({{ $order->id }},'{{ OrderStatus::ReadyForPickup }}')" />
                                <span for="test3"></span>
                        ...
                        <div class="order-col">
                            <label class="order-status-option status-option {{ $order->order_status == OrderStatus::Delivered ? "active" : "" }}">
                                <input id="test4" type="radio" name="order-status-option" {{ $order->order_status == OrderStatus::Delivered ? "checked" : "" }} onclick="changeOrderStatusNew({{ $order->id }},'{{ OrderStatus::Delivered }}')" />
                                <span for="test4"></span>
                                    ...
                                    <div class="table-content">
                                        <table>
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ trans('modal.order_detail.total') }} €{{ number_format($order->total_amount, 2) }} <br />
                                            ...
                                            @if($order->coupon_code)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <p class="text-uppercase">{{ $order->coupon_code ? $order->coupon_code : '-'}}</p>
                                                    </td>
                                                </tr>
                                            @endif
                                            @if($order->delivery_note)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <p class="text-underline">{{ $order->delivery_note ? $order->delivery_note : "-" }}</p>
                                                    </td>
                                                </tr>
                                            @endif
                                        </table>
                                    ...
                                    <div class="timing-details mb-3">
                                        ...
                                        <div class="timing-row">
                                            ...
                                            <div class="timing-col">
                                                ...
                                                <div class="text-uppercase">{{ date('H:i',strtotime(\Carbon\Carbon::parse($order->created_at)->addMinutes($orderDeliveryTime))) }}</div>
                                                <div class="t-box">+5</div>
                                            </div>
                                        </div>
                                    </div>
            ...
            <div class="modal-footer d-flex align-items-center justify-content-between">
                <div class="clearfix">
                    <button type="button" class="btn btn-outline-danger text-danger" data-bs-dismiss="modal">{{ trans('modal.order_detail.cancel') }}</button>
                    <a class="btn btn-outline-secondary ms-2 text-secondary d-inline-flex align-items-center gap-3" target="_blank"
                       href="{{ route('orders.printLabel', ['order_id' => $order->id]) }}">{{ trans('rest.food_order.print') }}</a>
                </div>

                <div class="clearfix d-flex align-items-center">
                    @if($order->delivery_note)
                    <h3 class="note d-flex align-items-center">{{ trans('modal.order_detail.see_note') }} <img class="ms-1" src="{{ asset('images/note.png') }}"
                            alt="" /></h3>
                    @endif
                    <h3 class="note d-flex align-items-center ms-3">{{ $order->payment_type == PaymentType::Cash && $order->order_status != OrderStatus::Delivered ? trans('modal.order_detail.unpaid_order') : trans('modal.order_detail.paid_order') }} <img class="ms-1" src="{{ asset('images/note.png') }}"
                            alt="" /></h3>
