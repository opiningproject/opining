...
@if(count($orders))
    @foreach($orders as $key => $order)
        ...
        <div class="foodorder-box-list-item d-flex order-{{ $order->id }}"
             onclick="orderDetailArchive({{ $order->id }})" id="order-{{ $order->id }}" data-id="{{ $order->id }}">
            <div class="details w-100 d-flex flex-column gap-3">
                ...
                <div
                    ...
                    <div class="icontext-item d-flex align-items-center gap-1">
                        ...
                        <div
                            class="text">{{ $order->order_type == OrderType::Delivery ? trans('rest.food_order.delivery'):trans('rest.food_order.take_away') }} </div>
                    </div>
                    <div class="icontext-item d-flex align-items-center gap-1">
